...
class ResultService {
  ...
  async getQuestionAnalytics(filters, user) {
    ...
    const pipeline = [
      { $match: query },
      { $unwind: '$questions' },
      ...(filters.questionId ? [{ $match: { 'questions.questionId': filters.questionId } }] : []),
      // UPDATED: Filter by question type and difficulty using flat structure
      ...
      ...(filters.difficulty ? [{ $match: { 'questions.difficulty': filters.difficulty } }] : []),
      {
        $group: {
          ...
          questionTitle: { $first: '$questions.title' }, // UPDATED: direct field access
          ...
          correctAttempts: { $sum: { $cond: ['$questions.isCorrect', 1, 0] } },
          averageTime: { $avg: '$questions.timeSpent' },
          averagePoints: { $avg: '$questions.pointsEarned' }, // UPDATED: pointsEarned
          successRate: { $avg: { $cond: ['$questions.isCorrect', 1, 0] } }
        }
      },
      {
